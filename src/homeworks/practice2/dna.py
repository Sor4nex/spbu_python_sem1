...
def get_txts_from_user() -> tuple:
    ...
    while not os.path.isfile(source_file_name):
        ...
        source_file_name = formate_file_extension(input())
    print(
        "Введите имя .txt файла, куда будет выгружен результат преобразования последовательности:"
    )
...
def operation_dna_insert(dna: str, start_pattern: str, insert_snippet: str) -> str:
    ...
    return (
        dna[:last_index_of_start_pattern]
        + insert_snippet
        + dna[last_index_of_start_pattern:]
    )


def operation_dna_replace(
    dna: str, pattern_to_replace: str, replace_snippet: str
) -> str:
