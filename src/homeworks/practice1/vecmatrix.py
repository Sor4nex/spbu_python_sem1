def vector_calculator() -> None:
    ...
    if len(vector1) != len(vector2) or len(vector1) < 2 or len(vector2) < 2:
        ...
        return None
    vector1_len, vector2_len = calculate_vector_length(vector1), calculate_vector_length(vector2)
...
def create_vector() -> tuple:
    ...
    while any(not element.isdigit() for element in usr_inp):
        usr_inp = input('Произошла ошибка, введите координаты через пробел: ').strip().split(' ')
...
def calculate_vectors_angle(scalar: float, vector1_len: float, vector2_len: float) -> float:
    result_angle = scalar / (vector1_len * vector2_len)
    return result_angle


def matrix_calculator() -> None:
    usr_inp = input('Введите через пробел два числа. Кол-во строк в матрицах, затем стоблцов: ').strip().split(' ')
    while any(not element.isdigit() for element in usr_inp) or len(usr_inp) != 2:
        usr_inp = input('Ошибка, введите два числа через пробел. Кол-во строк и столбцов: ').strip().split(' ')
...
def create_matrix(height: int, width: int) -> list:
    ...
    for i in range(height):
        ...
        while any(not element.isdigit() for element in single_str) or len(single_str) != width:
...
if __name__ == "__main__":
    print('Добро пожаловать в приложение для вычислений операций, связанных с матрицами и векторами.')
