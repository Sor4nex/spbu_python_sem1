    ...
    for i in range(len(args)):
        ...
        if ".txt" in args[i]:
            if parsed_arguments["operation"] is None:
                parsed_arguments[
                    "Error"
                ] = "no operation was invocated or invalid argument order"
                break
            ...
            if parsed_arguments["operation"] is None:
                ...
                break
            if (parsed_arguments["operation"] == "wc" and args[i] == "-n") or (
                (
                    parsed_arguments["operation"] == "head"
                    or parsed_arguments["operation"] == "tail"
                )
                and args[i] in ["-l", "-w", "-m"]
            ):
                parsed_arguments["Error"] = "invalid argument given"
            parsed_arguments[args[i]] = (
                int(args[i + 1])
...
def operation_wc(parsed_arguments: dict) -> list:
    ...
    with open(parsed_arguments["txt_file"], "r", encoding="utf-8") as given_file:
        ...
        while single_string != "":
            ...
            counters["words"] += (
                len(single_string.split(" ")) if "" != single_string.strip() else 0
            )
...
def operation_head(parsed_arguments: dict) -> list:
    ...
    if "-n" in parsed_arguments.keys():
        needs["lines"] = (
            parsed_arguments["-n"] if parsed_arguments["-n"] is not None else 10
        )
    elif "-c" in parsed_arguments.keys():
        needs["lines"] = None
        needs["size"] = (
            parsed_arguments["-c"] if parsed_arguments["-c"] is not None else 10
        )
...
def operation_tail(parsed_arguments: dict) -> list:
    ...
    if len(parsed_arguments.keys()) == 2:
        ...
        if "-n" in parsed_arguments.keys():
            frst_str = (
                int(parsed_arguments["-n"])
                if parsed_arguments["-n"] is not None
                else 10
            )
            strs_output = usr_input_txt.split("\n")[-frst_str:]
            for i in range(len(strs_output)):
            ...
            frst_symbol = (
                int(parsed_arguments["-c"])
                if parsed_arguments["-c"] is not None
                else 10
            )
...
if __name__ == "__main__":
    ...
    if parsed_arguments["operation"] == "wc":
        ...
        for string in (
            operation_head(parsed_arguments)
            if parsed_arguments["operation"] == "head"
            else operation_tail(parsed_arguments)
        ):
            print(string)
