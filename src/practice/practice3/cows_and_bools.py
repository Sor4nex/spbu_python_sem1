...
def start_game() -> None:
    numbers_options = [''.join(number) for number in
                       permutations(['1', '2', '3', '4', '5', '6', '7', '8', '9', '0'], r=4) if
                       number[0] != '0']
    ...
    while user_number_guess != goal_number:
        user_number_guess = input(f'Попытка №{user_attempts + 1}. Введите предполагаемое число: ')
        user_attempts += 1
    ...
    print(f'Ура! Получилось! Загаданное число было {goal_number}. Вы угадали с {user_attempts} попытки.')
