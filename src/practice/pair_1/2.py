def rearrange_array() -> None:
    ...
    while None in given_array or len(given_array) < 3:
        usr_inp = input(
            "Один или несколько элементов не являются целыми числами, введите числа через пробел: "
        )
        given_array = [
            int(elem) if elem.isdigit() else None for elem in usr_inp.split(" ")
        ]
    ...
    m_index = int(m_index)
    given_array.reverse()
    given_array[:-m_index], given_array[-m_index:] = (
        given_array[-m_index:],
        given_array[:-m_index],
    )
    given_array.reverse()
